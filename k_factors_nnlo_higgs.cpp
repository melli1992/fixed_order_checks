...
double vegas_higgs_NNLO_gg_zdep(double *k, size_t dim, void *params){
	...
	double result = higgs_LO_factor()*(higgs_NNLO_gg_reg(z)*jacobian*real(pdf_sum_gg(x,tau/z))/z+higgs_NNLO_gg_plus(z)*(jacobian*real(pdf_sum_gg(x,tau/z))/z-(1.-tau)*real(pdf_sum_gg(tau+k[1]*(1.-tau),tau))));
...
double vegas_higgs_NNLO_gg_zdep_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_gg_reg(z)*jacobian*real(fit_sum_gg(x,tau/z))/z+higgs_NNLO_gg_plus(z)*(jacobian*real(fit_sum_gg(x,tau/z))/z-(1.-tau)*real(fit_sum_gg(tau+k[1]*(1.-tau),tau))));
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_gg_zdepcorr(double *k, size_t dim, void *params){
	...
	double result =  higgs_LO_factor()*(higgs_NNLO_gg_plus(z))*jacobian*(-pdf_sum_gg(x,tau));
...
double vegas_higgs_NNLO_gg_zdepcorr_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau; 
	double x = tau+k[1]*jacobian;
	double result =  higgs_LO_factor()*(higgs_NNLO_gg_plus(z))*jacobian*(-real(fit_sum_gg(x,tau)));
...
double vegas_higgs_NNLO_gg_delta(double *k, size_t dim, void *params){
	(void)(dim);
	(void)(params);
	double result = higgs_LO_factor()*higgs_NNLO_gg_delta()*real(pdf_sum_gg(k[0],tau));
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_gg_delta_fit(double *k, size_t dim, void *params){
	(void)(dim);
	(void)(params);
	double result = higgs_LO_factor()*higgs_NNLO_gg_delta()*real(fit_sum_gg(k[0],tau));
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_gg_LP(double *k, size_t dim, void *params){
	...
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_gg_plus(z))*(jacobian*pdf_sum_gg(tau/z+k[1]*jacobian,tau/z)/z - (1.-tau)*pdf_sum_gg(tau+k[1]*(1.-tau),tau));
}
double vegas_higgs_NNLO_gg_LP_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_gg_plus(z))*(jacobian*real(fit_sum_gg(tau/z+k[1]*jacobian,tau/z))/z - (1.-tau)*real(fit_sum_gg(tau+k[1]*(1.-tau),tau)));
}
double vegas_higgs_NNLO_gg_power(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_gg_expansion(z, fp->power))*(jacobian*pdf_sum_gg(tau/z+k[1]*jacobian,tau/z)/z);
}
double vegas_higgs_NNLO_gg_power_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_gg_expansion(z, fp->power))*(jacobian*real(fit_sum_gg(tau/z+k[1]*jacobian,tau/z))/z);
}

/// and the NNLO functions for the gg channel
...
double higgs_NNLO_gg_reg(double x){
	...
	+1./x*(9.*(38.*pow(x,2)-20.*pow(x,3)+18.*x-39.*pow(x,4)+14.+7.*pow(x,5))/(1.-pow(x,2))*Li3(x)-18.*pow((pow(x,2)+x+1),2)/(1.+x)*S12(pow(x,2))
			...
			+9./2.*(2.*pow(x,4)-15.*pow(x,2)-10.*x-5.)/(1.+x)*Li2(-x)*log(x)-9./4.*(59.+177.*pow(x,2)-166.*pow(x,3)+59.*pow(x,4)-118.*x)/(1.-x)*log(x)*pow(log(1.-x),2)
...
double higgs_NNLO_gg_delta(){
	return pow(alphas_muR/M_PI,2)*((11399./144.+133./2.*zeta2-165./4.*zeta3-9./20.*pow(zeta2,2)+19./8.*Lt)+nF*(-1189./144.+5./6.*zeta3-5./3.*zeta2+2./3.*Lt));
	
}
// power expansion, checked the coefficients with the mathematica code
double higgs_NNLO_gg_expansion(double x, int power){
	if(power==1){
		return pow(alphas_muR/M_PI,2)*(3147 - 684*pow(M_PI,2) + 2*nF*(-49 + 6*pow(M_PI,2)) + 3*log(1 - x)*(-1983 + 61*nF + 180*pow(M_PI,2) - 24*log(1 - x)*(-30 + nF + 36*log(1 - x))) - 6318*zeta3)/36.;
	}
	if(power==2){
		return pow(alphas_muR/M_PI,2)*(1 - x)*(-771.8333333333334 + (263*pow(M_PI,2))/4. + nF*(13.73611111111111 - (8*pow(M_PI,2))/9.) + (log(1 - x)*(12291 - 209*nF - 360*pow(M_PI,2) + 4*log(1 - x)*(-765 + 16*nF + 432*log(1 - x))))/12. + 351*zeta3);
	}
	if(power==3){
		return pow(alphas_muR/M_PI,2)*(pow(-1 + x,2)*(44828 - 965*nF - 2376*pow(M_PI,2) + 24*log(1 - x)*(-1682 + 28*nF + 873*log(1 - x))))/96.;
	}
	if(power==4){
		return pow(alphas_muR/M_PI,2)*pow(1 - x,3)*(-757.1067708333334 + 29*pow(M_PI,2) + nF*(11.765432098765432 - (5*pow(M_PI,2))/9.) + (log(1 - x)*(4788 - 56*nF - 90*pow(M_PI,2) + log(1 - x)*(-1125 + 20*nF + 432*log(1 - x))))/6. + (351*zeta3)/2.);
	}
	if(power==5){
		return pow(alphas_muR/M_PI,2)*pow(-1 + x,4)*(-327.39152083333335 + (391*pow(M_PI,2))/20. + nF*(5.709187885802469 - (5*pow(M_PI,2))/9.) + log(1 - x)*(485.4 - (272*nF)/45. - 15*pow(M_PI,2) + (-84.225 + (10*nF)/3.)*log(1 - x) + 72*pow(log(1 - x),2)) + (351*zeta3)/2.);
	}
	if(power==6){
		return pow(alphas_muR/M_PI,2)*pow(1 - x,5)*(-233.03654166666666 + (609*pow(M_PI,2))/40. + nF*(4.4048391203703705 - (76*pow(M_PI,2))/135.) + log(1 - x)*(413.56 - (3197*nF)/675. - 15*pow(M_PI,2) + (-35.925 + (152*nF)/45.)*log(1 - x) + 72*pow(log(1 - x),2)) + (351*zeta3)/2.);
	}
	if(power==7){
		return pow(alphas_muR/M_PI,2)*pow(-1 + x,6)*(-187.7184452137998 + (3457*pow(M_PI,2))/280. + nF*(3.8538812200806247 - (77*pow(M_PI,2))/135.) + (388.5057142857143 - (36731*nF)/9450. - 15*pow(M_PI,2))*log(1 - x) + (-3.642857142857143 + (154*nF)/45.)*pow(log(1 - x),2) + 72*pow(log(1 - x),3) + (351*zeta3)/2.);
	}
	if(power==8){
		return pow(alphas_muR/M_PI,2)*pow(1 - x,7)*(-157.21313551536687 + (1135*pow(M_PI,2))/112. + nF*(3.5441843393442034 - (109*pow(M_PI,2))/189.) + (379.50204081632654 - (85373*nF)/26460. - 15*pow(M_PI,2))*log(1 - x) + ((5319 + 872*nF)*pow(log(1 - x),2))/252. + 72*pow(log(1 - x),3) + (351*zeta3)/2.);
	}
	if(power==9){
		return pow(alphas_muR/M_PI,2)*pow(-1 + x,8)*((-948743967014 + 23810115581*nF - 5174400*(-11421 + 800*nF)*pow(M_PI,2))/7.112448e9 - ((-9994419 + 70717*nF + 396900*pow(M_PI,2))*log(1 - x))/26460. + (41.426785714285714 + (220*nF)/63.)*pow(log(1 - x),2) + 72*pow(log(1 - x),3) + (351*zeta3)/2.);
	}
	if(power==10){
		return pow(alphas_muR/M_PI,2)*pow(1 - x,9)*(-113.44591817050895 + (809*pow(M_PI,2))/120. + nF*(3.2171579041421463 - (95*pow(M_PI,2))/162.) + log(1 - x)*(379.74150793650796 - (7451*nF)/3402. - 15*pow(M_PI,2) + log(1 - x)*(58.7875 + (95*nF)/27. + 72*log(1 - x))) + (351*zeta3)/2.);
	}
	if(power==-1){
		return higgs_NNLO_gg_reg(x)-pow(alphas_muR/M_PI,2)*((pow(-1 + x,2)*(44828 - 965*nF - 2376*pow(M_PI,2) + 24*log(1 - x)*(-1682 + 28*nF + 873*log(1 - x))))/96. + (3147 - 684*pow(M_PI,2) + 2*nF*(-49 + 6*pow(M_PI,2)) + 3*log(1 - x)*(-1983 + 61*nF + 180*pow(M_PI,2) - 24*log(1 - x)*(-30 + nF + 36*log(1 - x))) - 6318*zeta3)/36. + pow(-1 + x,6)*(-187.7184452137998 + (3457*pow(M_PI,2))/280. + nF*(3.8538812200806247 - (77*pow(M_PI,2))/135.) + (388.5057142857143 - (36731*nF)/9450. - 15*pow(M_PI,2))*log(1 - x) + (-3.642857142857143 + (154*nF)/45.)*pow(log(1 - x),2) + 72*pow(log(1 - x),3) + (351*zeta3)/2.) + pow(-1 + x,8)*((-948743967014 + 23810115581*nF - 5174400*(-11421 + 800*nF)*pow(M_PI,2))/7.112448e9 - ((-9994419 + 70717*nF + 396900*pow(M_PI,2))*log(1 - x))/26460. + (41.426785714285714 + (220*nF)/63.)*pow(log(1 - x),2) + 72*pow(log(1 - x),3) + (351*zeta3)/2.) + pow(1 - x,7)*(-157.21313551536687 + (1135*pow(M_PI,2))/112. + nF*(3.5441843393442034 - (109*pow(M_PI,2))/189.) + (379.50204081632654 - (85373*nF)/26460. - 15*pow(M_PI,2))*log(1 - x) + ((5319 + 872*nF)*pow(log(1 - x),2))/252. + 72*pow(log(1 - x),3) + (351*zeta3)/2.) + pow(-1 + x,4)*(-327.39152083333335 + (391*pow(M_PI,2))/20. + nF*(5.709187885802469 - (5*pow(M_PI,2))/9.) + log(1 - x)*(485.4 - (272*nF)/45. - 15*pow(M_PI,2) + (-84.225 + (10*nF)/3.)*log(1 - x) + 72*pow(log(1 - x),2)) + (351*zeta3)/2.) + pow(1 - x,5)*(-233.03654166666666 + (609*pow(M_PI,2))/40. + nF*(4.4048391203703705 - (76*pow(M_PI,2))/135.) + log(1 - x)*(413.56 - (3197*nF)/675. - 15*pow(M_PI,2) + (-35.925 + (152*nF)/45.)*log(1 - x) + 72*pow(log(1 - x),2)) + (351*zeta3)/2.) + pow(1 - x,9)*(-113.44591817050895 + (809*pow(M_PI,2))/120. + nF*(3.2171579041421463 - (95*pow(M_PI,2))/162.) + log(1 - x)*(379.74150793650796 - (7451*nF)/3402. - 15*pow(M_PI,2) + log(1 - x)*(58.7875 + (95*nF)/27. + 72*log(1 - x))) + (351*zeta3)/2.) + pow(1 - x,3)*(-757.1067708333334 + 29*pow(M_PI,2) + nF*(11.765432098765432 - (5*pow(M_PI,2))/9.) + (log(1 - x)*(4788 - 56*nF - 90*pow(M_PI,2) + log(1 - x)*(-1125 + 20*nF + 432*log(1 - x))))/6. + (351*zeta3)/2.) + (1 - x)*(-771.8333333333334 + (263*pow(M_PI,2))/4. + nF*(13.73611111111111 - (8*pow(M_PI,2))/9.) + (log(1 - x)*(12291 - 209*nF - 360*pow(M_PI,2) + 4*log(1 - x)*(-765 + 16*nF + 432*log(1 - x))))/12. + 351*zeta3));
	}
...
double vegas_higgs_NNLO_qg_zdep(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qg_reg(z)*jacobian*real(pdf_sum_qg(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qg_zdep_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qg_reg(z)*jacobian*real(fit_sum_qg(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qg_power(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qg_expansion(z, fp->power)*jacobian*real(pdf_sum_qg(x,tau/z))/z);
}
double vegas_higgs_NNLO_qg_power_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qg_expansion(z, fp->power)*jacobian*real(fit_sum_qg(x,tau/z))/z);
}
...
double higgs_NNLO_qg_reg(double x){
	return  pow(alphas_muR/M_PI,2)*
		...
		+0.*nF*1./x*((1./18.*pow(x,2)-1./9.*x+1./9.)*pow(log(1.-x),2)+(-38./27.*x+19./27.*pow(x,2)+29./27.)*log(x)-209./81.*x+265./162. //klopt
...
double vegas_higgs_NNLO_qq_zdep(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qq_reg(z)*jacobian*real(pdf_sum_qq(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qq_zdep_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qq_reg(z)*jacobian*real(fit_sum_qq(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qq_power(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qq_expansion(z, fp->power)*jacobian*real(pdf_sum_qq(x,tau/z))/z);
}
double vegas_higgs_NNLO_qq_power_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qq_expansion(z, fp->power)*jacobian*real(fit_sum_qq(x,tau/z))/z);
}
...
double vegas_higgs_NNLO_qqp_zdep(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qqp_reg(z)*jacobian*real(pdf_sum_qqNI(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qqp_zdep_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qqp_reg(z)*jacobian*real(fit_sum_qqNI(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qqp_power(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qqp_expansion(z, fp->power)*jacobian*real(pdf_sum_qqNI(x,tau/z))/z);
}
double vegas_higgs_NNLO_qqp_power_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qqp_expansion(z, fp->power)*jacobian*real(fit_sum_qqNI(x,tau/z))/z);
}
//NNLO coefficients  qq' channel
// klopt met code
...
double vegas_higgs_NNLO_qqbar_zdep(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qqbar_reg(z)*jacobian*real(pdf_sum_qqbar(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qqbar_zdep_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; 
	double x = tau/z+k[1]*jacobian;
	double result = higgs_LO_factor()*(higgs_NNLO_qqbar_reg(z)*jacobian*real(fit_sum_qqbar(x,tau/z))/z);
	if (isnan(result)){return 0;}
	else{return result;}
}
double vegas_higgs_NNLO_qqbar_power(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qqbar_expansion(z, fp->power)*jacobian*real(pdf_sum_qqbar(x,tau/z))/z);
}
double vegas_higgs_NNLO_qqbar_power_fit(double *k, size_t dim, void *params){
	...
	double jacobian = 1.-tau/z; //needed to transform the boundary dependent terms
	double x = tau/z+k[1]*jacobian;
	return higgs_LO_factor()*(higgs_NNLO_qqbar_expansion(z, fp->power)*jacobian*real(fit_sum_qqbar(x,tau/z))/z);
}
