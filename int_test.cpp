...
double PDFprodQG(double a, double b, double scale) {
    double prod=0.;
    double Qf[6] = {-1./3.,2./3.,-1./3.,2./3.,-1./3.,2./3.};
    for(int i=1; i<=5; i++){
        prod += pow(Qf[i-1],2)*(xfx(a,scale)[6+i]/a*xfx(b,scale)[6]/b + xfx(a,scale)[6]/a*xfx(b,scale)[6+i]/b + xfx(a,scale)[6-i]/a*xfx(b,scale)[6]/b + xfx(a,scale)[6]/a*xfx(b,scale)[6-i]/b);
        cout << prod << endl;
    }
    return prod;
}
...
double partonLumQG(double x, double xmin, double xmax, double scale) {
    double jacx = xmax - xmin;
    return 1/(xmin/jacx+x)*PDFprodQG(xmin+jacx*x,xmin/(xmin+jacx*x),scale);
}
...
static int QG(const int *ndim, const cubareal xx[],
              const int *ncomp, cubareal ff[], void *userdata) {
#define z_var xx[1]
#define x_var xx[0]
#define qg ff[0]
    ...
    lterms = prefactor(S,Q)*as/(4.*pi)*Tf*(1-7*pow(z,2)+6*z)*jacz*partonLumQG(x_var, tau/z, 1., Q)/z;
    return 0;
}
...
int main(){
    ...
    Vegas(NDIM2, NCOMP, QG, params,
          NVEC, EPSREL, EPSABS, verbose, SEED,
          MINEVAL, MAXEVAL, NSTART, NINCREASE, NBATCH,
          GRIDNO, STATEFILE, &spin, &neval, &fail, integral, error, prob);
    double qgres = integral[0];
    double qger = error[0];
    ...
    cout << endl << "QG-CHANNEL" << endl;
    cout << "..........................................................................." << endl;
    cout << "Total qg-channel contribution: " << qgres  << " pb/GeV^2  (" << qger << ")" << endl;
    cout <<endl << "---------------------------------------------------------------------------" << endl;
